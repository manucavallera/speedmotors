...
- `smoke_marina.sh` — Smoke test marina — turnera/guardería/proveeduría + endpoints nuevos (~1669 tok)
...
- `.claudeignore` — === DEPENDENCIAS Y BUILDS === (~296 tok)
...
- `Dockerfile` — Docker container definition (~114 tok)
...
- `0006_purchase_order_mixto.sql` — SQL: tables: purchase_orders, purchase_order_items (~283 tok)
- `0007_financing_currency.sql` (~18 tok)
- `0008_reservations.sql` — SQL: tables: reservations (~261 tok)
- `0009_reminders.sql` — SQL: tables: reminders (~242 tok)
...
- `0013_missing_columns.sql` (~102 tok)
- `0014_reservation_extra_fields.sql` (~317 tok)
- `0015_transport_and_iva.sql` — Campos de transporte por venta/remito (condición IVA ya existe en clients.condicion_iva) (~178 tok)
- `0015_vehicle_photo.sql` (~20 tok)
- `0016_reservation_vehicle_lock.sql` — Reservas vinculan opcionalmente a un vehículo del inventario. (~142 tok)
...
- `alerts.service.ts` — Exports AlertsService (~2578 tok)
...
- `auth.module.ts` — Exports AuthModule (~200 tok)
...
- `seed.ts` — pool: seed (~984 tok)

## apps/backend/src/expenses/

- `expense.dto.ts` — Exports CreateExpenseDto, UpdateExpenseDto (~144 tok)
- `expenses.controller.ts` — Exports ExpensesController (~349 tok)
...
- `notifications.module.ts` — Exports NotificationsModule (~112 tok)
- `notifications.service.ts` — Exports NotificationsService (~1281 tok)
...
- `reports.controller.ts` — Exports ReportsController (~330 tok)
- `reports.module.ts` — Exports ReportsModule (~74 tok)
- `reports.service.ts` — Exports ReportsService (~2972 tok)
...
- `supplier.dto.ts` — Exports CreateSupplierDto, UpdateSupplierDto (~91 tok)
- `suppliers.controller.ts` — Exports SuppliersController (~378 tok)
- `suppliers.module.ts` — Exports SuppliersModule (~87 tok)
- `suppliers.service.ts` — Exports SuppliersService (~660 tok)
...
- `users.controller.ts` — Exports UsersController (~347 tok)
- `users.module.ts` — Exports UsersModule (~89 tok)
- `users.service.ts` — Exports UsersService (~854 tok)
...
- `SuppliersTable.tsx` — initials — renders table (~999 tok)
...
- `receipt.ts` — Exports generateInstallmentReceipt, generateAccountPaymentReceipt, generateGuarderiaReceipt, generat (~3562 tok)
...
- `ProveeduriaPage.tsx` — fmt (~1551 tok)
...
- `turnera.types.ts` — Lancha de guardería elegible para botadura (viene de /guarderia/units?status=en_guarderia) (~362 tok)
