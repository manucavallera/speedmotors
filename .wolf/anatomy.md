...
- `MEMORY.md` — Memory Index (~179 tok)
...
- `settings.json` (~334 tok)

## ../../.claude/projects/C--Users-coco-proyectos-speedmotors/memory/

- `feedback_effort.md` (~96 tok)
- `MEMORY.md` — Memory Index (~363 tok)
- `project_negro_blanco_pendiente.md` — Ya implementado (2026-05-05) (~453 tok)

## ./

- `.claudeignore` — .claude/sessions/** (~129 tok)
- `.gitignore` — Git ignore rules (~13 tok)
- `.mcp.json` (~83 tok)
- `AUDIT.md` — AUDIT — SpeedMotors (~3727 tok)
- `CLAUDE.md` — OpenWolf (~413 tok)
- `docker-compose.yml` — Docker Compose services (~236 tok)
- `package-lock.json` — npm lock file (~177904 tok)
- `package.json` — Node.js package manifest (~70 tok)

## .claude/

- `ARCHITECTURE_MAP.md` — Architecture Map -- SpeedMotors (~602 tok)
- `COMMON_MISTAKES.md` — Errores comunes -- SpeedMotors (~377 tok)
- `DOCUMENTATION_MAINTENANCE.md` — Mantenimiento de documentacion -- SpeedMotors (~416 tok)
- `LEARNINGS_INDEX.md` — Learnings & decisiones de diseño -- SpeedMotors (~267 tok)
- `QUICK_START.md` — Quick Start -- SpeedMotors (~368 tok)
- `settings.json` (~441 tok)
- `settings.local.json` — Declares WHERE (~1313 tok)
...
- `seed-test-data.md` — Seed completo SpeedMotors - testing exhaustivo (~1250 tok)
...
- `2026-05-01_setup-documentacion-y-features.md` — Session: Setup documentacion + features blanco/negro + precios (~997 tok)
...
- `package.json` — Node.js package manifest (~778 tok)
- `README.md` — Project documentation (~1282 tok)
- `tsconfig.build.json` — TypeScript build configuration (~29 tok)
...
- `0003_daily_mockingbird.sql` — SQL: tables: cash_movements, 4 alter(s) (~284 tok)
- `0004_invoice_serial.sql` — SQL: 4 alter(s) (~128 tok)
- `0005_billing_mixto.sql` (~77 tok)
- `0006_purchase_order_mixto.sql` (~51 tok)
- `0007_financing_currency.sql` (~18 tok)
- `0008_reservations.sql` — SQL: tables: reservations (~244 tok)
- `0009_reminders.sql` — SQL: tables: reminders (~186 tok)
- `0010_transfers.sql` — SQL: tables: transfers (~242 tok)
- `0011_client_payments.sql` — SQL: tables: client_payments (~158 tok)
- `0012_indexes.sql` — Performance indexes Sprint 2 (~166 tok)
- `apply_purchase_orders.sql` — Ejecutar cuando Docker esté levantado: (~291 tok)

## apps/backend/drizzle/meta/

- `_journal.json` (~185 tok)
- `0000_snapshot.json` (~6098 tok)
- `0001_snapshot.json` (~9908 tok)
- `0002_snapshot.json` (~9953 tok)
- `0003_snapshot.json` (~10724 tok)
...
- `auth.controller.ts` — Exports AuthController (~126 tok)
- `auth.module.ts` — Exports AuthModule (~180 tok)
- `auth.service.ts` — Exports AuthService (~295 tok)
- `jwt-auth.guard.ts` — Exports JwtAuthGuard (~46 tok)
- `jwt.strategy.ts` — Exports JwtStrategy (~170 tok)
- `roles.guard.ts` — Exports AdminGuard (~105 tok)

## apps/backend/src/cash/

- `cash.controller.ts` — Exports CashController (~389 tok)
- `cash.module.ts` — Exports CashModule (~74 tok)
- `cash.service.ts` — Exports CashService (~1423 tok)

## apps/backend/src/clients/

- `clients.controller.ts` — Exports ClientsController (~510 tok)
- `clients.module.ts` — Exports ClientsModule (~83 tok)
- `clients.service.ts` — Exports ClientsService (~1536 tok)
...
- `reservations.schema.ts` — Exports reservations (~467 tok)
- `sales.schema.ts` — Exports sales, saleItems, installments (~801 tok)
...
- `vehicles.schema.ts` — Exports vehicles (~363 tok)

## apps/backend/src/db/seeds/

- `seed-testing.ts` — pool: getAdminId, seedSuppliers, seedClients + 3 more (~9008 tok)
- `seed.ts` — pool: seed (~1002 tok)

## apps/backend/src/expenses/

- `expenses.controller.ts` — Exports ExpensesController (~316 tok)
- `expenses.module.ts` — Exports ExpensesModule (~83 tok)
- `expenses.service.ts` — Exports ExpensesService (~490 tok)

## apps/backend/src/products/

- `products.controller.ts` — Exports ProductsController (~504 tok)
- `products.module.ts` — Exports ProductsModule (~85 tok)
- `products.service.ts` — Exports ProductsService (~1323 tok)

## apps/backend/src/purchase-orders/

- `create-purchase-order.dto.ts` — Exports PurchaseOrderItemDto, CreatePurchaseOrderDto (~90 tok)
- `purchase-orders.controller.ts` — Exports PurchaseOrdersController (~346 tok)
...
- `quotes.controller.ts` — Exports QuotesController (~421 tok)
- `quotes.module.ts` — Exports QuotesModule (~100 tok)
- `quotes.service.ts` — Exports QuotesService (~1298 tok)

## apps/backend/src/reports/

- `reports.controller.ts` — Exports ReportsController (~312 tok)
- `reports.module.ts` — Exports ReportsModule (~74 tok)
- `reports.service.ts` — Exports ReportsService (~2849 tok)

## apps/backend/src/reservations/

- `create-reservation.dto.ts` — Exports CreateReservationDto (~296 tok)
- `reservations.controller.ts` — Exports ReservationsController (~363 tok)
- `reservations.module.ts` — Exports ReservationsModule (~103 tok)
- `reservations.service.ts` — Exports ReservationsService (~1084 tok)

## apps/backend/src/sales/

- `create-sale.dto.ts` — Exports SaleItemDto, CreateSaleDto (~366 tok)
- `sales.controller.ts` — Exports SalesController (~474 tok)
- `sales.module.ts` — Exports SalesModule (~78 tok)
- `sales.service.ts` — Exports SalesService (~2381 tok)

## apps/backend/src/stock-movements/

- `stock-movements.controller.ts` — Exports StockMovementsController (~204 tok)
...
- `suppliers.controller.ts` — Exports SuppliersController (~272 tok)
- `suppliers.module.ts` — Exports SuppliersModule (~87 tok)
- `suppliers.service.ts` — Zustand store (~464 tok)

## apps/backend/src/transfers/

- `create-transfer.dto.ts` — Exports CreateTransferDto (~272 tok)
- `transfers.controller.ts` — Exports TransfersController (~301 tok)
- `transfers.module.ts` — Exports TransfersModule (~97 tok)
- `transfers.service.ts` — Exports TransfersService (~1438 tok)
...
- `users.controller.ts` — Exports UsersController (~306 tok)
- `users.module.ts` — Exports UsersModule (~89 tok)
- `users.service.ts` — Exports UsersService (~543 tok)

## apps/backend/src/vehicles/

- `vehicles.controller.ts` — Exports VehiclesController (~462 tok)
- `vehicles.module.ts` — Exports VehiclesModule (~85 tok)
- `vehicles.service.ts` — Exports VehiclesService (~726 tok)

## apps/backend/test/

- `app.e2e-spec.ts` — API routes: GET (1 endpoints) (~216 tok)
- `jest-e2e.json` (~55 tok)
...
- `ClientFormModal.tsx` — toForm — renders form, modal (~927 tok)
- `ClientPaymentFormModal.tsx` — typeOptions — renders form, modal (~1120 tok)
- `ClientsTable.tsx` — ClientsTable — renders table (~1373 tok)

## apps/frontend/src/components/dashboard/

- `SalesBySellerWidget.tsx` — SalesBySellerWidget (~782 tok)
...
- `ProductFormModal.tsx` — toForm — renders form, modal (~2286 tok)
- `ProductsTable.tsx` — StockBadge — renders table (~2020 tok)
...
- `ReservationFormModal.tsx` — toForm — renders form, modal (~2583 tok)

## apps/frontend/src/components/sales/

- `SaleDetailModal.tsx` — invoiceColors — renders modal (~2475 tok)
...
- `ChangePasswordModal.tsx` — ChangePasswordModal — renders form, modal (~546 tok)
- `UserFormModal.tsx` — UserFormModal — renders form, modal (~841 tok)

## apps/frontend/src/hooks/

- `useAlerts.ts` — API routes: GET, POST, PUT, PATCH, DELETE (8 endpoints) (~1137 tok)
- `useAuth.ts` — Exports useAuth (~168 tok)
- `useClients.ts` — API routes: GET, POST, PUT, DELETE (4 endpoints) (~688 tok)
- `useProducts.ts` — API routes: GET, POST, PUT, DELETE (4 endpoints) (~1030 tok)
...
- `useSales.ts` — API routes: GET, POST (6 endpoints) (~891 tok)
...
- `remito.ts` — Exports generateRemito (~1054 tok)
- `reservation.ts` — Exports generateReservationPdf, printReservation (~2776 tok)

## apps/frontend/src/lib/reports/

- `presets.ts` — Exports METHOD_LABELS, PRESET_RANGES, getDefaultRange (~376 tok)

## apps/frontend/src/pages/

- `AlertsPage.tsx` — SectionBox (~2651 tok)
- `CashPage.tsx` — CashPage (~1017 tok)
- `ClientsPage.tsx` — ClientsPage (~759 tok)
- `DashboardPage.tsx` — KPICard (~3608 tok)
...
- `VehiclesPage.tsx` — VehiclesPage (~965 tok)
...
- `api-design.md` — API Design -- NestJS SpeedMotors (~399 tok)
- `authentication.md` — Authentication -- JWT + Passport + NestJS (~265 tok)
- `common-pitfalls.md` — Anti-patrones confirmados -- SpeedMotors (~418 tok)
- `database-patterns.md` — Database Patterns -- Drizzle + PostgreSQL (~440 tok)
- `deployment.md` — Deployment -- SpeedMotors (~300 tok)
- `performance.md` — Performance -- SpeedMotors (~325 tok)
